...
const getThread = async (req, res) => {
  ...
  try {
    ...
    } else if (userThreads === "verified") {
      query = query.where({ authorIsVerified: true });
...
const postComment = async (req, res) => {
  ...
  try {
    ...
    thread.comments.push({
      ...
      commenterIsVerified: author.isVerified,
...
const reportThread = async (req, res) => {
  try {
    const data = req.body;
    const thread = await Thread.findOne({ _id: data.reportedUser });
    console.log(thread.title);
    const url = `https://forum.revnitro.com/${data.reportedUser}`;
    const content = `<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
  ...
  <body>
    <div class="notification-container">
      <div class="notification-content">
        <p><strong>${data.userIdentity}</strong> has reported a thread due to <strong>${data.reason}</strong>:</p>
        <p>${thread.title}</p>
      </div>
      <a href="${url}" class="button-link">Click here to see</a>
      <div class="logo-container">
    ...
    const recipientEmails = [
      ...
      "dinesh@revnitro.com",
      "dinesh@asherdn.com",
      "ranjithkumar@asherdn.com",
    ];
    sendMail(recipientEmails.join(","), "Thread Report", content);
    res.json(true);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
...
const reportUser = async (req, res) => {
  try {
    const data = req.body;
    console.log(data);
    const url = `https://forum.revnitro.com/reported/${data.reportedUser}`;
    const content = `<!DOCTYPE html>
  <html lang="en">
    <head>
      <meta charset="UTF-8" />
    ...
    <body>
      <div class="notification-container">
        <div class="notification-content">
          <p><strong>${data.userIdentity}</strong> has reported a user due to <strong>${data.reason}</strong>:</p>
          <p>${data.reportedUser}</p>
        </div>
        <a href="${url}" class="button-link">Click here to see</a>
        <div class="logo-container">
          <img
    ...
    sendMail(recipientEmails.join(","), "User Report", content);
    res.json(true);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
